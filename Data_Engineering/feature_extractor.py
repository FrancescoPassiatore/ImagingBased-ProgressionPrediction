...
def make_lungmask(img, display=False):
 ...
 # Compute Lung Area in the slice
    ...
    return mask,lung_pixels, slice_lung_area, num_t_pixels, tissue_by_total, tissue_by_lung


def extract_histogram_features(img, mask):
    """
    Extracts Mean, Skew, and Kurtosis from the histogram of lung images.
    Arguments:
    - img: Image array (after preprocessing)
    - mask: Lung mask (1 for lung pixels, 0 for background)

    Returns:
    - mean: Mean of the pixel values in the lung region.
    - skewness: Skewness of the pixel values in the lung region.
    - kurtosis_value: Kurtosis of the pixel values in the lung region.
    """
    # Mask the image to focus on the lung pixels
    lung_pixels = img[mask == 1]  # Only pixels inside the lung mask

    # Calculate the mean of the lung pixels
    mean = np.mean(lung_pixels)

    # Calculate the skewness of the lung pixels' attenuation values
    skewness = skew(lung_pixels)

    # Calculate the kurtosis of the lung pixels' attenuation values
    kurtosis_value = kurtosis(lung_pixels)

    return mean, skewness, kurtosis_value

#Extract features patients
#Path train folder
tr_csv = pd.read_csv("C:/Users/frank/OneDrive/Desktop/Thesis - Progress Prediction/CodeDevelopment/osic-pulmonary-fibrosis-progression/train.csv")

train_dicoms = "C:/Users/frank/OneDrive/Desktop/Thesis - Progress Prediction/CodeDevelopment/osic-pulmonary-fibrosis-progression/train"
...
#Mean, Median, Skew, Kurthosis, HAA of non zero pixels in image and Height, BMI and chest circumference

#Why do we use percentiles also for feature extraction, we could leverage all slices since we have final average