...
def main():
    results_base_dir = Path(r"D:\FrancescoP\ImagingBased-ProgressionPrediction\Thesis_training\1_k_top\training_1")
    results_base_dir.mkdir(parents=True, exist_ok=True)
    
    # Define slice features cache path
    slice_features_cache = Path(r"D:\FrancescoP\ImagingBased-ProgressionPrediction\Thesis_training\1_k_top\slice_features.csv")
    ...
    # Extract or load CNN features
    print("\n" + "="*70)
    print("CNN FEATURES")
    print("="*70)
    
    if slice_features_cache.exists():
        print(f"✓ Loading cached slice features from: {slice_features_cache}")
        slice_features_df = pd.read_csv(slice_features_cache)
        print(f"  Loaded {len(slice_features_df)} slices")
        print(f"  Patients: {slice_features_df['patient_id'].nunique()}")
    else:
        print("Extracting CNN features (no cache found)...")
        ...
        # Save to cache
        print(f"\n✓ Saving slice features to cache: {slice_features_cache}")
        slice_features_cache.parent.mkdir(parents=True, exist_ok=True)
        slice_features_df.to_csv(slice_features_cache, index=False)
        print(f"  Saved {len(slice_features_df)} slices")
