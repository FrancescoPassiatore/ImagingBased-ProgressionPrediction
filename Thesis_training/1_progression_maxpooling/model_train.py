...
class ProgressionPredictionModel(nn.Module):
    ...
    def __init__(
        ...
        use_feature_branches: bool = True,  # NEW: separate processing branches,
        use_ktop: bool = False
    ):
        super().__init__()
    ...
    def predict_proba(self, 
        ...
        """
        # Use forward pass (already does pooling)
        logits = self.forward(slice_features, lengths, patient_features)  # (batch_size, 1)
        probs = torch.sigmoid(logits).squeeze(-1)  # (batch_size,)
        
        batch_size, max_slices, _ = slice_features.shape
        
        # Get pooling information for interpretation
        if self.pooling_type == 'max':
            # Find which slice was selected by max pooling
            mask = torch.arange(max_slices, device=slice_features.device)[None, :] < lengths[:, None]
            mask = mask.unsqueeze(-1)
            masked_features = slice_features.clone()
            masked_features[~mask.expand_as(slice_features)] = -1e9
            _, max_indices = masked_features.max(dim=1)  # (batch_size, feature_dim)
            # Take most common index across features as "most important slice"
            most_important_slice = torch.mode(max_indices, dim=1)[0]  # (batch_size,)
            
            return {
                'prob': probs,  # (batch_size,) - final prediction probability
                'pooling_type': 'max',
                'important_slice_idx': most_important_slice,  # (batch_size,) - which slice contributed most
                'num_slices': lengths  # (batch_size,) - number of slices per patient
            }
            ...
            mask = mask.unsqueeze(-1)
            attn_scores = attn_scores.masked_fill(~mask, -1e9)
            attn_weights = torch.softmax(attn_scores, dim=1).squeeze(-1)  # (batch_size, max_slices)
            
            # Most attended slice
            most_important_slice = attn_weights.argmax(dim=1)  # (batch_size,)
            
            return {
                'prob': probs,  # (batch_size,) - final prediction probability
                'pooling_type': 'attention',
                'attention_weights': attn_weights,  # (batch_size, max_slices) - attention per slice
                'important_slice_idx': most_important_slice,  # (batch_size,) - most attended slice
                'num_slices': lengths  # (batch_size,) - number of slices per patient
            }
...
def compute_upper_lower_percentage(topk_indices, lengths, split=0.5):
    """
    Args:
        topk_indices: Tensor (B, K)
        lengths: Tensor (B,)
    Returns:
    ...
    for i in range(topk_indices.shape[0]):
        total_slices = lengths[i].item()
        boundary = int(total_slices * split)

        idx = topk_indices[i]
        upper = (idx < boundary).sum().item()
        lower = (idx >= boundary).sum().item()
        total = idx.numel()
    ...
    def evaluate(self, dataloader) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                # Forward pass
                logits = self.model(cnn_features, lengths, patient_features).squeeze(-1)
                loss = self.criterion(logits, labels)
