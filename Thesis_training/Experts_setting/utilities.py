...
class IPFDataLoader:
    ...
    def __init__(self, ground_truth_path: str, features_path: str, npy_dir: str, demographics_path: str = None):
        ...
        if demographics_path:
            ...
            # Get unique patient demographics (first row per patient)
            self.df_demographics = df_demo.groupby('Patient').first().reset_index()[['Patient', 'Age', 'Sex', 'SmokingStatus']]
        else:
    ...
    def get_patient_data(self) -> Tuple[Dict, Dict]:
        ...
        for patient_id in df['PatientID'].unique():
            ...
            gt_label_row = df[df['PatientID'] == patient_id]['has_progressed']
            if len(gt_label_row) == 0:
                continue
            gt_label = 1 if gt_label_row.iloc[0] in [True, 'True', 1, '1'] else 0
        ...
        for _, row in features_df.iterrows():
            ...
            if self.df_demographics is not None:
                demo_row = self.df_demographics[self.df_demographics['Patient'] == patient_id]
                if len(demo_row) > 0:
                    age = float(demo_row['Age'].iloc[0])
                    sex_str = demo_row['Sex'].iloc[0]
                    sex = 1 if sex_str == 'Male' else 0
                    smoking_str = demo_row['SmokingStatus'].iloc[0]
                    # Map smoking status: Never=0, Ex-smoker=1, Currently smokes=2
                    smoking_map = {'Never smoked': 0, 'Ex-smoker': 1, 'Currently smokes': 2}
                    smoking_status = smoking_map.get(smoking_str, 0)
                else:
                    age, sex, smoking_status = 65.0, 0, 0
            else:
                age, sex, smoking_status = 65.0, 0, 0
