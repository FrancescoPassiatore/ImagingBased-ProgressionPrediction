...
def normalize_features_per_fold(
    ...
    if not available_hand and not available_demo:
        return result_df  # Nessuna feature da normalizzare
...
def create_feature_set_for_fold(
    ...
    # Initialize encoding_info (will be populated if demographics are normalized)
    encoding_info = {}
    ...
    if len(patient_level_cols) > 1:
        ...
        if hand_to_add or demo_to_add:
            ...
            # Extract encoding info for passing to dataloaders
...
def train_single_fold(
    ...
    # Create model with SLICE-LEVEL PREDICTION architecture
    print(f"\nInitializing model (slice-level predictions):")
...
def main():
    results_base_dir = Path(r"D:\FrancescoP\ImagingBased-ProgressionPrediction\Thesis_training\1_progression\ablation_study_best_config_3fold_stratified")
