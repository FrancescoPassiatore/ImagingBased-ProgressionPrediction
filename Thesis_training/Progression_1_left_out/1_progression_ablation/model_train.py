...
class ProgressionPredictionModel(nn.Module):
    """
    Complete model for IPF progression prediction with separate branches
    for hand-crafted and demographic features
    """
    ...
    def __init__(
        ...
        print(f"Model feature dimensions:")
        print(f"  CNN features (per slice): {cnn_feature_dim}")
        print(f"  Hand-crafted features (patient-level): {hand_feature_dim}")
        print(f"  Demographic features (patient-level): {demo_feature_dim}")
        
        # Separate processing branches for different feature types
        self.use_hand = hand_feature_dim > 0
        self.use_demo = demo_feature_dim > 0
        
        # Hand-crafted feature branch
        if self.use_hand:
            self.hand_branch = nn.Sequential(
                nn.Linear(hand_feature_dim, 64),
        ...
        # Demographic feature branch
        if self.use_demo:
            self.demo_branch = nn.Sequential(
                nn.Linear(demo_feature_dim, 32),
        ...
        # Total input to classifier is CNN + processed hand + processed demo
        self.total_input_dim = cnn_feature_dim + hand_output_dim + demo_output_dim
        
        print(f"  Branch output dimensions:")
        if self.use_hand:
            print(f"    Hand-crafted branch: {hand_feature_dim} -> {hand_output_dim}")
        if self.use_demo:
            print(f"    Demographic branch: {demo_feature_dim} -> {demo_output_dim}")
        print(f"  Total input to classifier (per slice): {self.total_input_dim}")
    ...
    def forward(
        ...
        Args:
            ...
            hand_features: (batch_size, hand_feature_dim) - hand-crafted features
            demo_features: (batch_size, demo_feature_dim) - demographic features
        Returns:
            logits: (batch_size, 1)
        """
        batch_size, max_slices, cnn_dim = slice_features.shape
        
        # Process hand-crafted features through dedicated branch
        if self.use_hand and hand_features is not None:
            hand_processed = self.hand_branch(hand_features)  # (batch_size, 64)
            # Expand to match each slice
            hand_expanded = hand_processed.unsqueeze(1).expand(batch_size, max_slices, -1)
        else:
            hand_expanded = None
        
        # Process demographic features through dedicated branch
        if self.use_demo and demo_features is not None:
            demo_processed = self.demo_branch(demo_features)  # (batch_size, 32)
            # Expand to match each slice
            demo_expanded = demo_processed.unsqueeze(1).expand(batch_size, max_slices, -1)
        else:
            demo_expanded = None
        
        # Concatenate all features for each slice
        features_to_concat = [slice_features]
        if hand_expanded is not None:
            features_to_concat.append(hand_expanded)
        if demo_expanded is not None:
            features_to_concat.append(demo_expanded)
        
        combined_features = torch.cat(features_to_concat, dim=-1)  # (batch_size, max_slices, total_dim)
        
        # Reshape to process all slices at once
        # (batch_size, max_slices, total_dim) -> (batch_size * max_slices, total_dim)
        all_slices = combined_features.view(batch_size * max_slices, self.total_input_dim)
        
        # Classify each slice independently
        slice_logits = self.classifier(all_slices)  # (batch_size * max_slices, 1)
        
        # Reshape back to separate patients and slices
        slice_logits = slice_logits.view(batch_size, max_slices)  # (batch_size, max_slices)
        
        # Create mask for valid slices (handle variable-length sequences)
        mask = torch.arange(max_slices, device=slice_features.device)[None, :] < lengths[:, None]
        
        # Set invalid slices to very negative value (will be ignored in max)
        slice_logits_masked = slice_logits.clone()
        slice_logits_masked[~mask] = -1e9
        
        # Take MAX logit across slices for each patient
        max_logits, max_indices = slice_logits_masked.max(dim=1)  # (batch_size,)
        
        return max_logits.unsqueeze(-1)  # (batch_size, 1)
    
    def predict_proba(self, 
                     slice_features: torch.Tensor, 
                     lengths: torch.Tensor,
                     hand_features: torch.Tensor = None,
                     demo_features: torch.Tensor = None) -> Dict:
        """
        Return probabilities instead of logits
        Also returns per-slice probabilities for interpretation
        """
        batch_size, max_slices, cnn_dim = slice_features.shape
        
        # Process hand-crafted features through dedicated branch
        if self.use_hand and hand_features is not None:
            hand_processed = self.hand_branch(hand_features)
            hand_expanded = hand_processed.unsqueeze(1).expand(batch_size, max_slices, -1)
        else:
            hand_expanded = None
        ...
        # Process demographic features through dedicated branch
        if self.use_demo and demo_features is not None:
            demo_processed = self.demo_branch(demo_features)
            demo_expanded = demo_processed.unsqueeze(1).expand(batch_size, max_slices, -1)
        else:
            demo_expanded = None
        ...
        all_slices = combined_features.view(batch_size * max_slices, self.total_input_dim)
        slice_logits = self.classifier(all_slices)
        slice_logits = slice_logits.view(batch_size, max_slices)
        ...
        # Mask invalid slices
        mask = torch.arange(max_slices, device=slice_features.device)[None, :] < lengths[:, None]
        slice_probs_masked = slice_probs.clone()
        slice_probs_masked[~mask] = 0
        
        # Max probability across slices
        max_probs, max_indices = slice_probs_masked.max(dim=1)
        
        return {
            'max_prob': max_probs,  # (batch_size,) - final prediction
            'slice_probs': slice_probs_masked,  # (batch_size, max_slices) - all slices
            'max_slice_idx': max_indices  # (batch_size,) - which slice had max prob
        }
...
class ModelTrainer:
    ...
    def train_epoch(self, dataloader) -> Dict[str, float]:
        ...
        for batch in dataloader:
            cnn_features = batch['cnn_features'].to(self.device)  # (B, max_slices, cnn_feat_dim)
            ...
            logits = self.model(cnn_features, lengths, hand_features, demo_features).squeeze(-1)  # (B,)
    ...
    def evaluate(self, dataloader) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                # Forward pass
                logits = self.model(cnn_features, lengths, hand_features, demo_features).squeeze(-1)
...
def train_single_fold(features_df: pd.DataFrame,
    ...
    # Verifica che corrisponda alla configurazione
    if 'feature_dim' in config:
        if config['feature_dim'] != actual_total_dim:
            print(f"\nWARNING: Config feature_dim ({config['feature_dim']}) != Actual ({actual_total_dim})")
            print(f"   Updating config to match actual dimensions...")
            config['feature_dim'] = actual_total_dim
