...
class ProgressionPredictionModel(nn.Module):
    ...
    def __init__(
        ...
        print(f"\nProgressionPredictionModel Configuration:")
    ...
    def forward(self, features: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        """
        Flexible forward pass for both slice-level and patient-level
        
        Args:
            features: 
                - If training_mode='slice': (batch_size, feature_dim) - individual slices
                - If training_mode='patient': (batch_size, max_slices, feature_dim) - patients
            lengths: (batch_size,) - only needed for patient mode
        
        Returns:
            logits: (batch_size, 1) - progression logits
        """
        ...
        if self.training_mode == 'slice':
            ...
            #Patient level
            # Pool slices first (max pooling), then classify
            # features: (batch_size, max_slices, feature_dim)
            
            batch_size, max_slices, feature_dim = features.shape
            ...
            # Mask out invalid slices
            masked_features = features.clone()
            masked_features[~mask.expand_as(features)] = -1e9
            
            # Max pooling across slices
            pooled_features, _ = masked_features.max(dim=1)  # (batch_size, feature_dim)
    ...
    def predict_proba(self, slice_features: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        """
        batch_size, max_slices, feature_dim = slice_features.shape
        
        # Get all slice predictions
        all_slices = slice_features.view(batch_size * max_slices, feature_dim)
        ...
        slice_probs_masked[~mask] = 0  # Set invalid to 0
...
class ModelTrainer:
    ...
    def train_epoch_slice_level(self, dataloader) -> Dict[str, float]:
        ...
        for batch in dataloader:
            features = batch['features'].to(self.device)  # (B, feat_dim) - individual slices
            labels = batch['labels'].to(self.device).float()  # (B,)
            
            # For slice-level mode, features are already in correct shape (B, feat_dim)
            # No need to reshape since model expects 2D input in slice mode
            
            # Apply label smoothing
            labels_smooth = self._apply_label_smoothing(labels)

            # Pass features directly (B, feat_dim) and dummy lengths
            lengths = torch.ones(features.shape[0], dtype=torch.long, device=self.device)
            logits = self.model(features, lengths).squeeze(-1)  # (B,)
    ...
    def evaluate_patient_level(self, dataloader) -> Dict[str, float]:
        """Evaluate on validation/test set"""
        # Save original mode and temporarily switch to patient-level
        original_mode = self.model.training_mode
        self.model.eval()
        self.model.training_mode = 'patient'
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                # Forward pass
                logits = self.model(features, lengths).squeeze(-1)
                probs = torch.sigmoid(logits).cpu().numpy()

                all_patient_probs.extend(probs)
                all_patient_labels.extend(labels.cpu().numpy())  # Fixed: was extending itself
        
        # Restore original training mode
        self.model.training_mode = original_mode
        ...
        metrics = {
            'loss': 0.0,  # Not computed for patient-level eval
            'auc': roc_auc_score(all_patient_labels, all_patient_probs) if len(np.unique(all_patient_labels)) > 1 else 0.5,
            'acc': accuracy_score(all_patient_labels, all_patient_probs > 0.5),
            'f1': f1_score(all_patient_labels, all_patient_probs > 0.5, zero_division=0),
            'predictions': all_patient_probs,
    ...
    def fit(
        ...
        for epoch in range(epochs):
            # Train
            train_metrics = self.train_epoch_slice_level(train_loader_slices)
            
            # Validate
            val_metrics = self.evaluate_patient_level(val_loader_patients)
...
def train_single_fold(features_df: pd.DataFrame,fold_data: dict,fold_idx: int,config: dict,results_dir: Path,resume_from_checkpoint: bool = True):
    ...
    if resume_from_checkpoint and checkpoint_path.exists():
        ...
        is_complete = (
            ...
            'val_auc' in checkpoint
        )
        
        if is_complete:
            print(f"\n✓ Fold {fold_idx} already completed! Loading saved results...")
            return {
                'fold_idx': fold_idx,
    ...
    train_dataset_slices = SliceLevelDataset(
        features_df,
        train_ids,
        max_slices_per_patient=config.get('max_slices_per_patient', None)  # NEW!
    )
    
    train_loader_slices = DataLoader(
    ...
    # Compute SLICE-LEVEL class weights
    class_weights = compute_class_weights_slices(features_df, train_ids)
    
    # === CREATE PATIENT-LEVEL DATALOADERS (for validation/test) ===
    _, val_loader_patients, test_loader_patients = create_dataloaders(
        features_df,
        train_ids=train_ids,
        val_ids=val_ids,
        test_ids=test_ids,
    ...
    print(f"  Training mode: slice-level")
    ...
    model = ProgressionPredictionModel(
        ...
        training_mode='slice'  # Start in slice mode
    )
    ...
    # === TRAINING ===
    print(f"\n{'='*70}")
    print("TRAINING (Slice-level) & VALIDATION (Patient-level)")
    print("="*70)
    
    best_val_auc = trainer.fit(
        train_loader_slices=train_loader_slices,
        val_loader_patients=val_loader_patients,
    ...
    # === VALIDATION ANALYSIS (Patient-level) ===
    print(f"\n{'='*70}")
    print("VALIDATION SET THRESHOLD ANALYSIS (Patient-level)")
    print("="*70)
    
    val_results = trainer.evaluate_patient_level(val_loader_patients)
    ...
    # === TEST EVALUATION (Patient-level) ===
    print(f"\n{'='*70}")
    print("TEST SET EVALUATION (Patient-level)")
    print("="*70)
    
    test_results = trainer.evaluate_patient_level(test_loader_patients)
