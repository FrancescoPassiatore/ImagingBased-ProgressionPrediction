...
CONFIG = {
    ...
    "results_save_dir": Path(r"D:\FrancescoP\ImagingBased-ProgressionPrediction\Thesis_training\1_progression_slice_level\results_slice_level_strat_full"),
    
    # Model parameters
    'feature_dim': 2048,
    'aggregation': 'max',  # 'mean', 'max', 'attention', 'combined'
    'hidden_dims': [128,64],  # Increased capacity to avoid severe bottleneck
    'dropout': 0.6,  # Increased from 0.3 to reduce overfitting
    'use_batch_norm': True,
    
    # Training parameters
    'batch_size': 32,
    'learning_rate':1e-3,  # Increased from 1e-5 to escape local minima
    'weight_decay': 0.05,  # Increased from 0.001 for more regularization
    'epochs': 100,
    'early_stopping_patience': 20,  # Reduced from 20 to stop earlier

    'label_smoothing': 0.15,  # Added label smoothing to prevent overconfident predictions
    'use_scheduler': True,
...
def main():
    ...
    slice_features_df = feature_extractor.extract_features_patient_grouping(
        ...
        patients_per_batch=2,  # Reduced to 2 to avoid GPU OOM
        max_slices_per_forward=32,  # Process max 32 slices at once
        save_path=None
    )
