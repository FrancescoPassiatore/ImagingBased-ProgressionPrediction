...
class CNNFeatureExtractor:
    ...
    def extract_features_patient_grouping(self, patient_data, patients_per_batch=4, max_slices_per_forward=32, save_path=None):
        """
        Extract features from CT slices grouped by patient, ensuring all patient slices are in same batch.
        ...
        max_slices_per_forward : int
            Maximum number of slices to process in a single forward pass (to avoid GPU OOM)
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Extracting features"):
                ...
                # Process in smaller sub-batches if needed to avoid GPU OOM
                total_slices = images.size(0)
                all_batch_features = []
                
                for start_idx in range(0, total_slices, max_slices_per_forward):
                    end_idx = min(start_idx + max_slices_per_forward, total_slices)
                    sub_batch = images[start_idx:end_idx].to(self.device)
                    
                    # Extract features for sub-batch
                    sub_features = self.model(sub_batch)
                    sub_features = sub_features.view(sub_features.size(0), -1)  # Flatten
                    sub_features = sub_features.cpu().numpy()
                    all_batch_features.append(sub_features)
                ...
                features = np.concatenate(all_batch_features, axis=0)
...
#DATASET FOR SLICE LEVEL PROGRESSION

class SliceLevelDataset(Dataset):
    """
    Dataset that returns individual slices with patient-level labels
    For slice-level training with patient-level supervision
    """
    ...
    def __init__(
        ...
        feature_prefix: str = 'cnn_feature_',
        max_slices_per_patient: int = None  # NEW: limit slices
    ):
        """
        Args:
            features_df: DataFrame with CNN features per slice
            patient_ids: List of patient IDs to include
            feature_prefix: Prefix for CNN feature columns
            max_slices_per_patient: If set, randomly sample this many slices per patient
        """
        self.feature_prefix = feature_prefix
        ...
        # Sample slices if max_slices_per_patient is set
        if max_slices_per_patient is not None:
            sampled_slices = []
            for patient_id in patient_ids:
                patient_slices = self.df[self.df['patient_id'] == patient_id]
                n_slices = len(patient_slices)
                
                if n_slices > max_slices_per_patient:
                    # Randomly sample slices
                    sampled = patient_slices.sample(n=max_slices_per_patient, random_state=42)
                else:
                    # Use all slices if fewer than max
                    sampled = patient_slices
                
                sampled_slices.append(sampled)
            
            self.df = pd.concat(sampled_slices, ignore_index=True)
            
            print(f"\n  Slice sampling applied:")
            print(f"    Max slices per patient: {max_slices_per_patient}")
            print(f"    Total slices after sampling: {len(self.df)}")
        
        # Get feature columns
        self.feature_cols = [col for col in self.df.columns if col.startswith(feature_prefix)]
        self.feature_dim = len(self.feature_cols)
        
        # Ensure all feature columns are numeric (convert any object types)
        for col in self.feature_cols:
            if self.df[col].dtype == 'object':
                print(f"  WARNING: Converting feature column '{col}' from object to float")
                self.df[col] = pd.to_numeric(self.df[col], errors='coerce')
        
        # Check for NaN values after conversion
        nan_count = self.df[self.feature_cols].isna().sum().sum()
        if nan_count > 0:
            print(f"  WARNING: Found {nan_count} NaN values in features after conversion")
            print(f"           Filling with 0.0")
            self.df[self.feature_cols] = self.df[self.feature_cols].fillna(0.0)
        
        # Count classes
        n_progression = self.df['gt_has_progressed'].sum()
        n_total = len(self.df)
        
        print(f"\nSliceLevelDataset initialized:")
        print(f"  Total slices: {n_total}")
        print(f"  From patients: {self.df['patient_id'].nunique()}")
        print(f"  Feature dimension: {self.feature_dim}")
        print(f"  Progression slices: {n_progression} ({n_progression/n_total*100:.1f}%)")
        print(f"  Non-progression slices: {n_total - n_progression} ({(n_total-n_progression)/n_total*100:.1f}%)")
    ...
    def __getitem__(self, idx):
        ...
        # CNN features (single slice)
        # Convert to float explicitly to handle any object types
        feature_values = row[self.feature_cols].values
        try:
            feature_values = feature_values.astype(np.float32)
        except (ValueError, TypeError) as e:
            print(f"\nERROR converting features to float for patient {row['patient_id']}, slice {idx}")
            print(f"  Feature types: {row[self.feature_cols].dtypes}")
            print(f"  Feature values: {feature_values[:5]}...")  # Show first 5
            raise e
        
        features = torch.tensor(feature_values, dtype=torch.float32)
        
        # Label (patient-level, same for all slices from this patient)
        label = torch.tensor(row['gt_has_progressed'], dtype=torch.float32)
...
def collate_slice_batch(batch: List[Dict]) -> Dict:
    """Simple collate for slice-level training"""
...
def compute_class_weights_slices(features_df: pd.DataFrame, patient_ids: List[str]) -> torch.Tensor:
    """
    Compute class weights for SLICE-LEVEL training (more aggressive)
    """
    # Get all slices from these patients
    slice_labels = features_df[features_df['patient_id'].isin(patient_ids)]['gt_has_progressed']
    
    pos_count = slice_labels.sum()
    neg_count = len(slice_labels) - pos_count
    total = len(slice_labels)
    
    # Aggressive weighting for slice-level imbalance
    weight_pos = (total / (2 * pos_count)) * 2.0  # Double boost
    weight_neg = total / (2 * neg_count)
    
    print(f"\nSlice-level class distribution:")
    print(f"  Progression slices: {pos_count} ({pos_count/total*100:.1f}%)")
    print(f"  Non-progression slices: {neg_count} ({neg_count/total*100:.1f}%)")
    print(f"\nSlice-level class weights:")
    print(f"  Weight for progression: {weight_pos:.2f}")
    print(f"  Weight for non-progression: {weight_neg:.2f}")
