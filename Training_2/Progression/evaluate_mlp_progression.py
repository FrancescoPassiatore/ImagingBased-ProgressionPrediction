def save_combined_fvc52_and_progression_summary(fvc52_summary_path, progression_summary_path, output_path):
    ...
    def norm_key(x):
        x = str(x).lower()
        x = x.replace('mlp', '').replace('features', '').replace('full', 'full').replace('+', '').replace('handcrafted', 'handcrafted').replace('demographics', 'demographics')
...
"""
Evaluate Progression Prediction from MLP Corrector FVC@52 Predictions
=====================================================================

Uses the FVC@52 predictions from each MLP corrector feature type to classify progression
...
def evaluate_approach_progression(approach_key, approach_name, config, ground_truth_labels):
    ...
    for fold_idx in range(config['n_folds']):
        ...
        # Remove patients without valid FVC@52 (should already be filtered, but double-check)
        initial_count = len(fold_preds)
...
def main():
    print("="*80)
    print("MLP CORRECTOR PROGRESSION PREDICTION EVALUATION")
    print("="*80)
    ...
    # Save summary
    summary_path = CONFIG['results_dir'] / 'mlp_progression_prediction_summary.csv'
    summary_df.to_csv(summary_path, index=False)
    print(f"\n✓ Saved summary: {summary_path}")

    # Save detailed predictions for each approach
    for approach_key, results in all_results.items():
